...
<div align="center" id="d1">
    ...
    <table border="1">
        ...
        %>
        <tr><td>扶贫日志ID</td><td>日志提交时间</td><td>日志详细信息</td><td>附件</td></tr>
        <%
    ...
    <%
        ...
        if(pageUtil.getCurrentPage()!=1){
            out.println("<a href=toPlanLog?pageNumber="+(pageUtil.getCurrentPage()-1)+">上一页</a> ");
        }
        //显示超链接
        for(int i=1;i<=pageUtil.getPageCount();i++){
            out.println("<a href=toPlanLog?pageNumber="+i+">["+i+"]</a>");
        }
        //下一页
        if(pageUtil.getCurrentPage()!=pageUtil.getPageCount()){
            out.println("<a href=toPlanLog?pageNumber="+(pageUtil.getCurrentPage()+1)+">下一页</a>");
        }
    %>
    <a href="toPlan?pageNumber=1">返回上一层</a>
</div>
</body>
</html>
